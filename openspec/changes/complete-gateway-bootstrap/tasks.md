...
- [ ] 7.1 Verify the existing `vpnctl-v2-gateway` and `vpnctl-v2-node` fixture identity, image, resource profile, ownership, and stopped clean state, then journal every host/VM mutation in `docs/v2/HOST_CHANGELOG.md`; verify no production host and no old evidence directory is touched.
