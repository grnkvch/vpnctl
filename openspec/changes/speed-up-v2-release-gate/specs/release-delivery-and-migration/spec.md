...
- **THEN** a ready and explicitly triggered already-open tracked Gateway command whose temporary restart policy and TLS/HTTP control connection were prepared before measured load injects the outage after its guest-local delay, reuses the validated connection for the unavailable response, starts no new Lima shell, Python/TLS client, or `daemon-reload` at that boundary, and places the first client-visible impact inside the fixed scheduling sanity bound
