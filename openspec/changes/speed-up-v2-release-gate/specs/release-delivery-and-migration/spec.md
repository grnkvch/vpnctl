...
- **THEN** a ready and explicitly triggered already-open tracked Gateway command whose temporary restart policy was prepared before measured load injects the outage after its guest-local delay, no new Lima shell or `daemon-reload` enters the loaded fixture at that boundary, and the first client-visible impact is inside the fixed scheduling sanity bound
