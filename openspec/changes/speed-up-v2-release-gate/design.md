...
The first role-specific full run exposed three measurement artifacts. First, seven separate `systemctl show` calls per diagnostic sample allowed one one-second timeout during FRPS recovery to terminate the complete Gateway monitor and leave an empty resource document. The monitor now reads all declared units in one bounded call. A timeout or malformed response becomes a sanitized structured error in that sample, collection continues through the remaining timeline, and the aggregate is classified `invalid_measurement_evidence`; incomplete telemetry is neither a pass nor Gateway-saturation evidence. Second, Linux reports a one-GiB `mkswap` allocation as 4,096 fewer usable bytes because its first page is metadata. The versioned role contract records that exact kernel reservation and accepts only the closed interval from allocation-minus-one-page through allocation, while swap-use and every Gateway resource threshold remain unchanged. Third, the measured path now follows a successful ten-second unscored warm-up at the same request rates so separately asserted pre-disruption steady state does not measure QEMU/TLS cold start. The measured duration remains 300 seconds.
...
- **[A monitor timeout silently erases or fabricates capacity evidence]** → batch the bounded query, retain only structured error class/unit/offset, continue the timeline, and make any degraded required monitor an explicit non-passing measurement classification.
