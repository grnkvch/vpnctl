...
- [x] 12.7 Implement expose creation saga in tunnel-before-ingress order, immediate/deferred mode, degraded local-app behavior, URL/certificate/scp result, and no sensitive JSON path; verify ready and stopped-app E2E cases.
...
- [x] 14.1 Replace release signing with a canonical checksum-governed release manifest carrying binary/protocol/state ranges, OS/arch, pinned components/checksums/capabilities, apt compatibility, handshake-list version, and migration reversibility; remove the release signing key, detached signature asset, and embedded release trust anchor from build/install/update/rollback paths while retaining all non-release signatures; verify canonical encoding, corruption/tamper detection, unsupported-platform behavior, and the documented checksum-only trust boundary.
- [x] 14.2 Build a reproducible self-contained bundle for both roles and role-scoped local installation; verify normal init/apply/repair never fetch bundled components from upstream and an scp-transferred bundle installs successfully.
- [x] 14.3 Update the curl installer and local-asset path to consume exactly the standalone binary, self-contained bundle, and canonical checksum metadata, verify exact sizes/SHA-256 and bundle structure without a signature asset, and install the standard binary/bundle layout; verify corrupt downloads leave the existing installation untouched.
- [x] 14.4 Implement manual `update [version]` planning, latest-stable lookup only on request, gateway-first fleet compatibility, whole-bundle staging, state migration preview, component-by-component health, and expected interruption; verify no background/remote-node update occurs.
- [x] 14.5 Implement previous-bundle/state snapshots and `update rollback`, including irreversible-migration separate confirmation; verify compatible rollback restores exact versions/state and impossible rollback is refused before update.
- [x] 14.6 Ensure controller-only updates do not restart unchanged healthy data-plane units and changed units roll back independently; verify service restart counters and active forwarding during update tests.
...
- [x] 14.11 Implement recoverable `uninstall` with impact plan, gateway `--force`, node online revoke, node `--local-only`, DNS/network restoration, managed swap handling, state preservation, and binary-last removal; verify each role's online/offline cases.
- [x] 14.12 Implement typed-confirmed `purge` with state/preset/secret/cert/export removal and separately confirmed `--include-backups`; verify portable archives remain by default and purge has no hidden recovery promise.

## 15. One-Time v1 Migration

- [x] 15.1 Implement a read-only v1 installation inspector for cwd state, clients, keys, addresses, rulesets, generated artifacts, WireGuard, and known UFW rules; verify malformed/partial/unsupported installations yield a no-mutation report.
- [x] 15.2 Implement deterministic conversion to a fully validated staged v2 gateway state with preserved client IDs or mapped identities, keys, `10.66.0.x` addresses, profiles, and selector presets where compatible; verify conversion golden fixtures.
...
- [x] 15.4 Update the standalone migration script orchestration to verify the canonical checksum-governed v2 bundle without a release signature, then retain maintenance snapshot, role setup, known-UFW translation, watchdog-protected network activation, client validation, and accepted downtime; verify repeated dry runs, corrupt bundles, and interrupted phases are safe.
...
- [x] 16.3 Run equivalent standard-node and manual standard↔restricted test/switch/defer flows; verify no automatic fallback, one active steady-state transport, selected TCP/UDP fail-closed, and unrelated direct continuity.
- [x] 16.4 Run multi-node, multi-client, multi-expose isolation and authorization tests; verify no lateral networking, credential reuse, mapping impersonation, or cross-resource removal.
- [x] 16.5 Run credential lifecycle E2E for client rotation/revoke/delete and node rotation/revoke/delete/expired-certificate recovery; verify old generations fail and logical IDs/IPs/policies/exposes follow their contracts.
- [x] 16.6 Run ingress/tunnel failure E2E for app down, tunnel reconnect, gateway controller down, proxy reload, partial response, node revoke, and expose removal; verify correct `404`/`413`/`503`/`504` or connection close with no request replay.
- [x] 16.7 Run adversarial tests for invite/recovery replay, mTLS impersonation, stale generations, malicious tunnel mappings, symlink/permission attacks, secret redaction, firewall conflicts, IPv6/UDP/DNS leaks, and corrupt bundles/backups; verify all fail closed without foreign-resource damage.
- [x] 16.8 Run update/rollback and backup/restore E2E for same and changed public IPs across supported protocol windows; verify data-plane compatibility, explicit interruptions, and complete required-action lists.
- [x] 16.9 Run sustained minimum-gateway load for one Telegram node serving the defined several-hundred-user profile and five clients; verify controller idle RSS target, total memory/swap, CPU, disk, connection limits, latency, reconnect, and no OOM/deadlock.
- [x] 16.10 Document installation, happy paths, policy/DNS classification boundary, restricted UDP limitations, webhook registration responsibility, status/doctor/logging, backup/update/removal, migration, and troubleshooting; verify every documented command is executed in docs tests and no backlog feature is advertised.
- [ ] 16.11 Re-run the requirement traceability audit, strict OpenSpec validation, full mandatory Go/unit/integration/E2E/security/migration suites, and checksum-only three-asset release verification, explicitly confirming no detached release signature is published or required; keep the unchanged sustained capacity gate available only through explicit on-demand advisory execution; against an actually deployed gateway and node, manually verify supported Clash Mi profile import, selected TCP, proxy-bound DNS, UoT, strict wrong-host rejection, no fail-direct behavior, and reconnect, then use the token-safe harness to register the IP-only five-year public certificate with Telegram, receive and validate a real webhook request, and remove only the test-created registration; verify every remaining non-backlog release requirement is green before labeling the release v2.0.
- [x] 16.12 Make the deployed automated release gate resumable with append-only per-stage attempts bound to candidate, command/input fingerprints, and the Lima image digest; verify late failure resume, invalidation, immutable retry history, stopped-fixture cleanup, legacy evidence refusal, and final aggregation without weakening any underlying acceptance bound.
- [x] 16.13 Isolate the release builder's mandatory Go tests under deterministic `umask 022` while retaining restrictive `umask 077` for private work and later build/package subprocesses; verify phase boundaries, canonical output modes, and fail-before-publication cleanup behavior.
- [x] 16.14 Accept the canonical directory entry in the pinned official frp archive while retaining traversal rejection, and make migration planning plus release publication verify both role-specific bundle extraction paths before host mutation or `dist/` publication; verify the official archive shape, immutable failed-migration recovery, and fail-before-publication behavior.
