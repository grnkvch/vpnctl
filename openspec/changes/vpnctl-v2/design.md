...
The independently signed, versioned handshake-host document carried inside the release bundle contains an ordered list with stable candidate IDs and hostnames. Init probes TLS 1.3, certificate validity, reachability, and bounded latency and persists the first success. Runtime health never mutates the selection. Removing the release-artifact signature does not weaken this separate handshake-host signature or its verification.
...
The initial tunnel provider pins frp and configures one shared internal-only `frps`, one `frpc` child per node, `tcpMux` enabled, a two-second server-dial timeout, connection pooling disabled, dashboards and unused/public proxy types disabled. The explicit dial timeout bounds a failed restricted-transport attempt instead of inheriting frp's ten-second default. The stable node service wrapper observes only the authenticated loopback frpc status endpoint: after an already-connected tunnel remains transport-unavailable across a one-second guard (four polling intervals), it may recycle the provider child exactly once for that outage to reset a stale dial/backoff cycle while retaining enough of the eight-second recovery budget for a fresh authenticated handshake and stability probes. Readiness resets that allowance; if the one correction does not recover, the same child retains frp's indefinite bounded exponential retry. Local-upstream health failure does not trigger the guard, logical node/mapping identity does not change, and no standby or second dialer is introduced. Each expose gets a stable loopback port from the persisted internal TCP `20000-29999` allocator; disabled exposes retain their assignment until removal. Restore preserves free saved ports and builds a complete deterministic remap plan for unavailable or legacy out-of-range ports before any tunnel or ingress generation is published.
...
The canonical release manifest includes vpnctl version, supported control versions, state-schema range, target OS/arch, each bundled component version/hash/capabilities, compatible apt package ranges, handshake-host list version, and whether migrations are backward reversible. The self-contained bundle uses fixed versioned binary framing with no owner, timestamp, mode, or filesystem-order metadata: canonical manifest followed by exact path/size/artifact records in manifest order and exact EOF. The installer and updater verify the target platform, framing, every artifact size/hash, and provider archive structure before mutation.

Local installation consumes only an explicit regular non-symlink bundle path. It stages all manifest-governed artifacts before selecting a role, installs vpnctl and Mihomo for both roles, `frps` only for the gateway, and `frpc` only for the node. Existing byte-identical mode-`0755` targets are idempotent; any other target is a conflict, absent files are installed without replacement, and a partial invocation removes only paths it created. The installer reports the manifest-governed role-specific apt requirements but does not invoke apt or fetch upstream components. Therefore an `scp`-transferred bundle is self-contained for vpnctl-managed binaries, while Ubuntu repository access may still be required for OS packages.

The role-neutral bootstrap publishes exactly three assets: a standalone binary, the complete retained bundle, and canonical `release-checksums.txt`. The checksum metadata binds the requested release version and exact size/SHA-256 of both binary assets. The curl script downloads over HTTPS/TLS 1.2+, verifies everything in a private temporary directory, stages on the destination filesystem, and publishes the binary last. It also accepts an explicit local asset directory for trusted `scp` delivery. The standard retained paths are `/usr/local/bin/vpnctl` and root-only `/usr/local/lib/vpnctl/release/{vpnctl.bundle,checksums.txt}`. Init inspects the complete bundle read-only during plan, repeats verification and installs role-scoped binaries during apply, and persists only its component manifest; a plan/apply release mismatch fails before role layout mutation.

The release builder retains `umask 077` for its private work directory and build/package subprocesses, while the output writer explicitly applies the canonical asset modes. It runs mandatory Go verification in a child shell with deterministic `umask 022`. This prevents the restrictive builder policy from changing filesystem permissions observed by tests while ensuring the relaxed test mask cannot leak into later phases. A failing verification exits before build or publication and the builder removes only its private temporary work directory.

Before the builder publishes any asset into `dist/`, the maintainer verifier stages the complete bundle in a private temporary directory and prepares the install candidates for both gateway and node roles without touching system targets. This exercises the exact provider archive structure and both role-specific extraction paths in addition to checking framing and hashes. The standalone v1 migrator uses the same installability inspection during its read-only plan, so an unextractable provider archive fails before the maintenance snapshot or v1 service quiescence.

This v2.0 checksum-only design deliberately has no release signing key, detached signature asset, or embedded release trust anchor. SHA-256 protects against accidental corruption and against modification when checksum metadata is already trusted, but it cannot authenticate the publisher if an attacker replaces both an artifact and its checksum metadata. HTTPS release hosting or trusted SSH/`scp` transfer supplies the accepted delivery trust boundary. Cryptographically authenticated release publishing, including key rotation and recovery, is deferred to the backlog. This exception applies only to release delivery: enrollment transcripts, control PKI, backup authentication, tunnel credentials, public ingress TLS, and the independently signed handshake-host document retain their existing cryptographic contracts.
...
- **[DPI-resistant candidate changes upstream syntax or behavior]** → Pin binaries/config schemas in the canonical manifest, test the exact release bundle, expose only a transport behavior abstraction, and require an explicit upgrade plan.
- **[Checksum-only release metadata does not authenticate its publisher]** → Verify exact sizes/SHA-256 and complete bundle structure before mutation, use HTTPS or trusted SSH/`scp` as the v2.0 delivery trust boundary, document the limitation explicitly, and keep signed release publishing with key rotation/recovery in the backlog.
...
10. Run the complete mandatory E2E, adversarial security, fault-injection, and migration suites. Keep the unchanged minimum-capacity suite available as a separate on-demand advisory measurement. Release v2.0 only after every remaining non-backlog release requirement passes.
...
Development acceptance is not release acceptance. Task 16.9 owns the accepted sustained several-hundred-user plus five-client capacity boundary on the minimum gateway; later candidate measurements are explicit and advisory rather than automatic release blockers. Task 16.11 still owns actual supported Clash Mi behavior and real Telegram IP-only certificate/webhook registration against a deployed gateway/node. These gates do not authorize automatic fallback or silent changes to product behavior.

The deployed-service gate uses a versioned append-only attempt ledger beneath each new evidence directory. Each stage attempt records immutable input and result documents plus a private log. Its reuse fingerprint binds the candidate commit and version, the exact stage command, a conservative SHA-256 inventory of the complete tracked source tree (which includes every relevant script, fixture, configuration, spec, and test), and the pinned Lima image digest for VM-backed stages. This deliberately over-invalidates rather than risk reusing evidence after any tracked input change. `run-automated --resume` skips only matching successful mandatory attempts, appends a new attempt for a missing, failed, interrupted, or invalidated stage, and never rewrites an earlier attempt. `run-capacity [--resume]` applies the same evidence rules in a separate on-demand VM session and never participates in the final aggregate. The final aggregate names and hashes every selected mandatory automatic attempt and is published only after those stages pass and both exact fixtures are stopped. Evidence created by the earlier one-shot schema remains read-only.
