...
func RenderGatewayIdentityFirewall(state model.State, services GatewayIdentityFirewallServices) (GatewayFirewallArtifact, error) {
	if err := state.Validate(); err != nil {
		return GatewayFirewallArtifact{}, fmt.Errorf("validate gateway identity firewall state: %w", err)
	}
	if state.Host.Role != model.RoleGateway {
		return GatewayFirewallArtifact{}, fmt.Errorf("gateway identity firewall requires gateway state")
	}
	...
	return RenderGatewayFirewall(GatewayFirewallInput{
