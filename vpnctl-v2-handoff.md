...
Telegram harness packaged и проверен offline без provider call. Общий
read-only convergence planner детерминированно разделяет зарегистрированный
...
General `apply` принимает executable plan только от strict convergence planner,
группирует ресурсы по зарегистрированным pending operation ID и переносит
per-operation expected/desired generations до executor. Пересекающийся drift
является conflict до side-effect; после consent весь plan читается заново.
...
replay. Public certificate rotation выполняется только вручную, сразу и с
...
- После локального commit единый compiler строит generation-bound набор для
  ...
  Production runtime публикует весь набор и запускает units строго
