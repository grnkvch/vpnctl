...
frpc `v0.69.0` normalizes a declared `transport.poolCount = 0` to Login `pool_count = 1`. The local version-locked Login adapter accepts exactly that expected input after identity validation and returns otherwise unchanged Login content with `pool_count = 0` before frps creates the control session. Every other input is rejected. This uses frp's documented plugin content-replacement mechanism and keeps the provider's effective pool at zero; negative pool values are not used. The client also pins `transport.dialServerTimeout = 2`: the upstream default is ten seconds, which by itself exceeds vpnctl's eight-second reconnect gate when a restricted outer transport remains reachable while internal frps is unavailable. Production additionally wraps the single active child with a one-second, once-per-outage corrective recycle based on authenticated loopback status. Four 250-ms polling intervals confirm the unavailable state before correction and leave enough of the eight-second recovery budget for a fresh authenticated handshake plus stability probes. The original provider spike continues to prove native reconnect without process replacement; the sustained capacity gate separately measures the production wrapper and its overhead under the restricted failure shape.
