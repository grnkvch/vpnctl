...
func TestReleaseBuilderRejectsUnpinnedInputsAndLeavesNoPartialOutput(t *testing.T) {
	t.Parallel()
	...
	err := buildReleaseAssets(releaseBuildOptions{
		...
		OutputDir: output, MigrationBack: true,
	})
	if err == nil {
...
func TestReleaseBuilderRejectsIncompleteArguments(t *testing.T) {
	t.Parallel()
	if err := runReleaseBuild(nil); err == nil {
		t.Fatal("release builder accepted missing arguments")
	}
	if err := runReleaseBuild([]string{"unexpected"}); err == nil {
...
func TestReleaseBuilderWithPinnedProviderArchives(t *testing.T) {
	...
	if err := buildReleaseAssets(releaseBuildOptions{
		Version: "v2.0.0-test", VPNCTLPath: binaryPath, MihomoPath: mihomoPath, FRPPath: frpPath,
		OutputDir: secondOutput, MigrationBack: true,
	}); err != nil {
		t.Fatal(err)
	}
	...
	checksums, err := lifecycle.DecodeReleaseChecksums(checksumsBytes)
	if err != nil || checksums.Version != "v2.0.0-test" {
		t.Fatalf("output metadata=%+v err=%v", checksums, err)
	}
	...
	installer, err := lifecycle.NewReleaseBundleInstaller(installRoot, lifecycle.ReleasePlatform{
